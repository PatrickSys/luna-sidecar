...
**Status:** V1 verified; Phase 5 implementation complete locally; release evidence incomplete
...
- `start` treats the source-observed `thread.started` event from the exact `codex exec --json` invocation under the requested cwd and sandbox as the bounded readiness boundary. It proves invocation acceptance and a real provider session only; it does not prove task success, MCP health, or eventual completion. A missing, malformed, rejected, timed-out, or runner-dead readiness path returns a typed error, performs bounded owned-process cleanup, launches no fan-out work, and leaves no owned process alive. The normal human workflow has no separate `doctor` command.
...
| READY-01 | Bounded readiness | `start` reports usable only after a bounded source-observed `thread.started` provider-session boundary under the selected cwd/sandbox; it does not claim task success, MCP health, or eventual completion. Timeout, trust, sandbox, authentication, provider-start, malformed-readiness, and runner-death failures are typed and leave no owned process alive. |
